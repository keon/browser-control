...
browser session AND a logged-in user. `browser-control` connecting to the
...
1. **Coordinate click** (preferred — same shape as the rest of browser-control):
   ```python
   # Browser actions below map to browser-control CLI commands.
   btns = js("""
...
user's Chrome profile. They survive across `browser-control` runs and across
...
## Quick start (logged-in user, browser-control attached)
