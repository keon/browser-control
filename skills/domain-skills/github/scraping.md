...
- **Trending page selectors only work after navigation in the current browser state** — selectors that returned 0 results were run after the page had already navigated away. Navigate to the trending page, wait for load, then add a short hydration wait before querying.
