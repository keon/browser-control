...
> Browser Use is one of several cloud providers `browser-control` supports
> (`BROWSER_CONTROL_CLOUD_PROVIDER=browser-use`, the default; others include
...
`browser-control cloud-profiles` wraps the listing; prefer it unless you need
raw REST access.
...
that's outside the API surface and not useful from this workflow.
...
- **SDK features** — Browser Use ships official SDKs separately; this
  skill is the raw-HTTP path for power users.
