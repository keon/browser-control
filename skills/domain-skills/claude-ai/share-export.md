...
The share page **renders the chat content only when the user is logged into claude.ai**. Logged-out viewers see an app shell without the conversation DOM. Open the URL in a Chrome profile that's already signed in.
...
- Wrap extraction in an IIFE — `const`/`let` at top level can collide on re-run while iterating selectors.
...
Open the share URL, wait for the SPA to hydrate, then extract the structured turns from the DOM:

```bash
browser-control new-tab "https://claude.ai/share/<uuid>"
browser-control wait load 20
sleep 2
browser-control eval '
return JSON.stringify((() => {
  const userMsgs = [...document.querySelectorAll("[data-testid=user-message]")];
  if (!userMsgs.length) return {error: "no user messages found; check login"};
  let p = userMsgs[0];
  while (p && !p.contains(userMsgs[userMsgs.length - 1])) p = p.parentElement;
  const turns = [];
  for (const child of p.children) {
    const user = child.querySelector("[data-testid=user-message]");
    const assistant = child.querySelector(".font-claude-response");
    if (user) turns.push({role: "user", text: user.innerText.trim()});
    else if (assistant) turns.push({role: "assistant", text: assistant.innerText.trim()});
  }
  const header = document.querySelector("[data-testid=page-header]");
  const title = (header?.innerText.split("\n")[0] || document.title || "").trim();
  return {title, source_url: location.href, turns};
})())
'
```
