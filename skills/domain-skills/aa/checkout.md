...
For the PAN / CVV specifically, prefer CDP keystrokes via `browser-control type` — the checkout page's card-number tokenizer is happy with either, but keystrokes are a safer default if AA ever switches to a Spreedly/CyberSource iframe.
...
- **Tab title can contain automation markers** on real pages, so do not treat the browser title as site content.
