...
browser-control cdp Target.activateTarget '{"targetId":"<target-id>"}'
...
- `switch-tab` is **not enough** if the user expects Chrome to visibly change.
- `Target.activateTarget` is the CDP-side "show this tab".
- `tabs` can include `chrome://newtab/`; filter internal pages when you want only real pages.
